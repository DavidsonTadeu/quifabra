...
document.addEventListener('DOMContentLoaded', () => {
  // Authentication check (basic local for now, can be improved with Firebase Auth)
  ...
  if (isAdmin) {
    ...
    // Load customers (can also come from a users collection later)
    loadCustomers();
  }
});
...
function updateStats() {
  ...
  const aprovados  = allOrders.filter(o => o.status === 'Pago' || o.status === 'Em Separação' || o.status === 'Enviado' || o.status === 'Entregue').reduce((acc, o) => acc + (o.total || 0), 0);
  
  document.getElementById('stat-pedidos').textContent = allOrders.length;
  document.getElementById('stat-receita').textContent = 'R$ ' + totalGeral.toLocaleString('pt-BR',{minimumFractionDigits:2});
  document.getElementById('stat-aprovados').textContent = 'R$ ' + aprovados.toLocaleString('pt-BR',{minimumFractionDigits:2});
}
          ...
          <tr>
            ...
            <td>
              <select class="status-select" onchange="updateStatus('${o.id}', this.value)">
                <option value="Pendente" ${o.status==='Pendente'?'selected':''}>⏳ Pendente (Aguardando Pgto)</option>
...
async function loadCustomers() {
  // For now, getting unique customers from orders.
  // In a real app, you would query a 'users' collection in Firestore.
